...
public class ReadOperator implements BufferGetter,ChannelIO{
	private static Logger logger=Logger.getLogger(ReadOperator.class);
	...
	ReadOperator(ChannelContext context){
	...
	private boolean executeRead() {
		...
		synchronized(context){
			if(failure!=null){
				...
				if(orderOperator.isReadOrder()&&(totalCallbackLength+currentBufferLength)!=store.getPutBufferLength()){
					/* readOrderがありまだStoreから通知されていないbufferがある場合 */
...
//			queueSelect(State.connecting);
	...
	boolean asyncRead(Order order){
		if(currentBufferLength==0){
			if(this.state==State.selectReading){
	...
	boolean isSelectConnecting(){
		return (state==State.selectConnecting);
	}
	boolean isSelectReading(){
		return (state==State.selectReading);
	}
}
